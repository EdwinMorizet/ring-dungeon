...
@export_tool_button("regenerate", "RotateLeft") var regenerate_floor = _run_regenerate_action

# One-shot inspector action used to clear generated floor and debug visuals.
@export_tool_button("clear current", "Remove") var clear_current_floor = _run_clear_action

@export_group("Generation Preview (Editor)")
# One-shot inspector action that steps generation preview backward in editor mode.
@export_tool_button("Previous Step", "MoveLeft") var generation_debug_step_back = _step_back

# One-shot inspector action that steps generation preview forward in editor mode.
@export_tool_button("Next Step", "MoveRight") var generation_debug_step_forward = _step_next
...
func _run_editor_generation_preview(controller: DungeonFloorController) -> void:
	...
	if floor_config == null:
		...
		return
	_randomize_seed_if_needed(controller)
	var debug_timeline: DungeonGeneratorDebugTimeline = DungeonGeneratorDebugTimeline.new()
