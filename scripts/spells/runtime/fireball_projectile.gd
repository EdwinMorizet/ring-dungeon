...
@onready var _mesh_instance: Node3D = $Node3D
...
func configure(config: FireballConfig, origin: Vector3, direction: Vector3, shooter: PhysicsBody3D = null) -> void:
	...
	_shooter = shooter
	if _shooter != null: add_collision_exception_with(_shooter)
	...
	if _mesh_instance != null: _mesh_instance.scale = Vector3.ONE * projectile_size
...
func _apply_aoe_damage(excluded_target: Node = null, is_lesser: bool = false) -> void:
	...
	var aoe_scale: float = RING_BAND_CONSTANTS.LESSER_EXPLOSION_AOE_SCALE if is_lesser else RING_BAND_CONSTANTS.GREATER_EXPLOSION_AOE_SCALE
	var damage_scale: float = RING_BAND_CONSTANTS.LESSER_EXPLOSION_DAMAGE_SCALE if is_lesser else RING_BAND_CONSTANTS.GREATER_EXPLOSION_DAMAGE_SCALE
	...
	for result: Dictionary in results:
		...
		if collider_value is Node:
			...
			if collider_node == self or collider_node == excluded_target: continue
			if collider_node == _shooter:
				if is_lesser: continue
				var self_damage: int = maxi(int(roundf(float(_config.damage) * damage_scale * RING_BAND_CONSTANTS.SELF_GREATER_EXPLOSION_DAMAGE_SCALE)), 0)
				_apply_damage_to_target(collider_node, self_damage)
				continue
			if collider_node is EnemyBasic:
...
func _apply_damage_to_target(target: Node, amount: int) -> void:
	if target == null or amount <= 0: return
...
func _spawn_aoe_burst(aoe_radius: float, is_lesser: bool) -> void:
	...
	var burst: FireballAoeBurst = AOE_BURST_SCENE.instantiate() as FireballAoeBurst
	if not (burst is FireballAoeBurst):
		burst.queue_free()
		return
