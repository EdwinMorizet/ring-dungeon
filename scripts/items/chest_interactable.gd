...
func _process(delta: float) -> void:
	...
	if _is_opened or not _is_player_in_range:
		return
...
func _refresh_prompt_visibility() -> void:
	if _prompt_label == null:
		return
	_prompt_label.visible = _is_player_in_range and not _is_opened
...
func _update_prompt_visual(delta: float) -> void:
	...
	var distance: float = player_3d.global_position.distance_to(global_position)
